...
Outputs (in paper/figures/):
    ...
    joyplot.pdf          – ridgeline KDE of F1 at best config per metric (EMD excluded)
                           if --results-k5 is given, a dashed KDE for k=5 is overlaid
...
def load_results(path: Path) -> pd.DataFrame:
    """Load JSONL benchmark results; add config and metric_label columns."""
    rows = []
...
def plot_joyplot(
    ...
    If df_k5/agg_k5 are provided, a dashed KDE overlay for k=5 is drawn on each row
    using that dataset's own best config.
    """
    ...
    if df_best_k5 is not None:
        ...
        # Place legend on topmost axis, lower-right to avoid curve overlap
...
def plot_prototypes(n_classes: int = 10, n_proto: int = 2, seed: int = 42):
    ...
    # Full grid
    fig, axes = plt.subplots(n_proto, n_classes,
                             figsize=(n_classes * 1.1, n_proto * 1.3))
    for cls in range(n_classes):
        for row, img_idx in enumerate(chosen_per_class[cls]):
            ax = axes[row, cls]
            im = ax.imshow(X_train[img_idx], cmap=SHAP_CMAP)
            ax.axis("off")
            if row == 0:
                ax.set_title(str(cls), fontsize=10)
    fig.suptitle("Prototypical saliency maps (Sobel-filtered, SHAP colormap)",
                 fontsize=11, y=1.01)
    plt.colorbar(im, ax=axes, shrink=0.6, label="Attribution value")
    plt.tight_layout()
    out = FIGURES_DIR / "prototypes_grid.pdf"
    fig.savefig(out, dpi=300, bbox_inches="tight")
    fig.savefig(str(out).replace(".pdf", ".png"), dpi=150, bbox_inches="tight")
    print(f"  Saved {out}")
    plt.close(fig)

    # Per-class single-column figures
    per_class_dir = FIGURES_DIR / "prototypes_per_class"
    per_class_dir.mkdir(exist_ok=True)
    vmin, vmax = X_train.min(), X_train.max()
    for cls in range(n_classes):
        fig, axes_col = plt.subplots(n_proto, 1, figsize=(1.4, n_proto * 1.4))
        for row, img_idx in enumerate(chosen_per_class[cls]):
            axes_col[row].imshow(X_train[img_idx], cmap=SHAP_CMAP, vmin=vmin, vmax=vmax)
            axes_col[row].axis("off")
        plt.subplots_adjust(hspace=0.04, left=0, right=1, top=1, bottom=0)
        ...
        plt.close(fig)
    print(f"  Saved per-class figures → {per_class_dir}/")

    # Paper figure: 2 samples of class 0
    fig, axes_pair = plt.subplots(1, 2, figsize=(4, 2.2))
    for col, img_idx in enumerate(chosen_per_class[0]):
        ax = axes_pair[col]
        ...
        ax.set_xlabel(f"Sample {col + 1}", fontsize=9)
    fig.suptitle("Class 0 — two prototype saliency maps", fontsize=9, y=1.02)
    plt.tight_layout(rect=[0, 0, 0.88, 1])
    cbar_ax = fig.add_axes([0.90, 0.15, 0.03, 0.7])
    fig.colorbar(im, cax=cbar_ax, label="Attribution")
    out_pair = FIGURES_DIR / "prototypes_pair.pdf"
    fig.savefig(out_pair, dpi=300, bbox_inches="tight")
    fig.savefig(str(out_pair).replace(".pdf", ".png"), dpi=150, bbox_inches="tight")
...
def main():
    ...
    parser.add_argument("--results-k5", default=None,
                        help="Optional second JSONL file (k=5) for joyplot overlay")
    ...
    if multi_seed:
        plot_stability(agg)
    plot_joyplot(df, agg, df_k5=df_k5, agg_k5=agg_k5)
    plot_prototypes()
    print("Done.")
